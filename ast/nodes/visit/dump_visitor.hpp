...
struct DumpVisitor final : ExprVisitor,
                           DeclVisitor,
                           StmtVisitor,
                           ModuleVisitor {
  explicit DumpVisitor(std::ostream &os, CompilerContext &context)
      : os(os), stringInterner_(context.stringInterner),
        identInterner_(context.identInterner),
        numericInterner_(context.numericInterner) {}

  std::ostream &os;
  const Interner<> &stringInterner_;
  const Interner<> &identInterner_;
  const Interner<> &numericInterner_;
  ...
  template <class NodeT> void maybe_print_type_args(const NodeT &n) {
  ...
  void visit(ast::CharLiteralExpr &n) override {
    pad();
    os << "CharLiteral U+" << std::hex << static_cast<std::uint32_t>(n.v_)
       << std::dec << '\n';
  }
  ...
  void visit(ast::UnaryExpr &n) override {
    pad();
    os << "UnaryExpr op=" << unary_op_name(n.op) << " ("
  ...
  void visit(ast::BinaryExpr &n) override {
    pad();
    os << "BinaryExpr op=" << binary_op_name(n.op) << " ("
       << static_cast<int>(n.op) << ")\n";
  ...
  void visit(ast::AssignExpr &n) override {
    pad();
    os << "AssignExpr op=" << assign_op_name(n.op) << " ("
       << static_cast<int>(n.op) << ")\n";
  ...
  void visit(ast::FieldDecl &n) override {
    pad();
    os << "FieldDecl ";
    if (n.visibility_ == ast::Visibility::Publ)
      os << "pub ";
  ...
  void visit(LoadFnDecl &n) override {
    pad();
    os << "FunctionDecl name=";
    print_ident_sym(n.name_);
    os << " (sym#" << n.name_ << ")\n";
